...
def upgrade() -> None:
    ...
    CSV_PATH_EPISODES_DIAGNOSTICS = (
        BASE_PATH / "databases" / "postgresql" / "seeds" / "data" / "episodes_diagnostics.csv"
    )
    df = pd.read_csv(CSV_PATH_EPISODES_DIAGNOSTICS, dtype={"numero_episodio": str})

    if not {'numero_episodio', 'codigos'}.issubset(df.columns):
        raise ValueError("El CSV debe tener las columnas 'numero_episodio' y 'codigos'.")

    # Convertir string de lista a lista real
    import ast
    df['codigos'] = df['codigos'].apply(lambda x: ast.literal_eval(x) if isinstance(x, str) else x)

    episodes_map = {e.numero_episodio: e.id for e in session.execute(sa.select(sa.table('episodes', sa.column('id'), sa.column('numero_episodio')))).all()}
    diagnostics_map = {d.cie_code: d.id for d in session.execute(sa.select(sa.table('diagnostics', sa.column('id'), sa.column('cie_code')))).all()}
